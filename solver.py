...
def assign_customers_to_vehicles(customers, vehicles, max_capacity):
    ...
    for vehicle in vehicles:
        ...
        limit = min(int(remaining_capacity), len(ordered_customers_tuple))
...
def plan_vehicle_routing(warehouse, vehicle):
    ...
    init_distance, _, init_tour = greedy(points)

    # 2-opt solution

    best_distance, opt, best_tour = two_opt(points)

    init_obj = init_distance
    obj = best_distance

    print("* init (greedy) distance:", init_distance)
    print("* final (2-opt) distance:", best_distance)
    print("* best tour:", best_tour)
    print(f"SUMMARY init={init_distance} final={best_distance} opt={opt}")
    ...
    vehicle_tour = best_tour
    obj = best_distance

    return obj, opt, vehicle_tour, init_obj
...
def solve_vrp(warehouses, customers, is_plot):
    ...
    for vehicle in vehicles:
        obj, opt, vehicle_tour, init_obj = plan_vehicle_routing(warehouses[0], vehicle)

        init_total_cost += init_obj
        total_cost += obj

        output_data += 'vehicle' + str(vehicle.index + 1) + ': ' + ' '.join(
            str(int(v.index)) if hasattr(v, "index") else str(int(v))
            for v in vehicle_tour
        ) + '\n'
        if is_plot is True:
            plot_vehicle_tour(vehicle, vehicle_tour)

    print("* initial total cost (greedy):", init_total_cost)
    print("* final total cost (2-opt):", total_cost)

    output_data = (
            f"Init total cost: {init_total_cost}\n"
            f"Final total cost: {total_cost}\n"
            + output_data
    )
